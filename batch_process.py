...
class BrokenGradio:
    ...
    @staticmethod
    def progress_button(element: Callable, then: Callable, **kwargs):
        ...
        # Pass any additional kwargs to the 'then' call
...
class DepthGradio:
    ...
    def _render_worker(self, user: dict, image: ImageType, depth: ImageType, output_path: Path) -> Path:
        """The core rendering logic that creates a video from an image and depth map."""
        ...
        for preset in Animation.members():
            ...
            if preset_name in self.ui.animation and self.ui.animation[preset_name].get("enable"):
                preset_dict = self.ui.animation[preset_name]
                if user[preset_dict.enable]:
    ...
    def render_bulk(self, user: dict):
        """Processes a whole folder of images based on the current settings."""
        input_dir_str = user.get(self.ui.bulk.input_folder, "")
        output_dir_str = user.get(self.ui.bulk.output_folder, "")

        if not input_dir_str or not output_dir_str:
            yield {self.ui.bulk.log: "Please provide both input and output folder paths."}
            return
        ...
        if not input_dir.is_dir():
            yield {self.ui.bulk.log: f"Error: Input folder not found.\nPath: '{input_dir}'"}
            return

        try:
            output_dir.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            yield {self.ui.bulk.log: f"Error: Could not create output folder.\nPath: '{output_dir}'.\nDetails: {e}"}
            return

        image_files = sorted([p for p in input_dir.iterdir() if p.suffix.lower() in ['.png', '.jpg', '.jpeg', '.webp']])
        if not image_files:
            yield {self.ui.bulk.log: "No supported image files (.png, .jpg, .jpeg, .webp) found in the input folder."}
            return

        total_files = len(image_files)
        log_text = f"Found {total_files} images. Starting bulk process...\n"
        yield {self.ui.bulk.log: log_text}

        estimator = self._estimator(user)

        for i, image_path in enumerate(image_files):
            log_text += f"\n[{i + 1}/{total_files}] Processing '{image_path.name}'...\n"
            yield {self.ui.bulk.log: log_text}

            try:
                # 1. Load image
                log_text += "  - Loading image...\n"
                yield {self.ui.bulk.log: log_text}
                image = PILImage.open(image_path).convert("RGB")

                # 2. Estimate depth
                log_text += "  - Estimating depth map...\n"
                yield {self.ui.bulk.log: log_text}
                depth = estimator.estimate(image)

                # 3. Render video
                log_text += "  - Rendering video (this may take a while)...\n"
                yield {self.ui.bulk.log: log_text}
                output_video_path = output_dir / f"{image_path.stem}.mp4"

                self._render_worker(user=user, image=image, depth=depth, output_path=output_video_path)

                log_text += f"  - SUCCESS: Video saved to '{output_video_path.relative_to(output_dir.parent)}'\n"
                yield {self.ui.bulk.log: log_text}
    ...
    def launch(self,
        ...
        with gradio.Blocks(
            ...
            with gradio.Tabs():
                with gradio.TabItem("Single Process"):
                ...
                with gradio.TabItem("Bulk Process"):
                    self.ui.bulk = DotMap()
                    with gradio.Row():
                        with gradio.Column(variant="panel"):
                            gradio.Markdown("## Bulk Processing\n"
                                            "This tool processes all images in a folder using the settings defined in the 'Single Process' tab. "
                                            "Set up your desired animation, filters, and output settings first, then run the bulk process here.")
                            with gradio.Group():
                                self.ui.bulk.input_folder = gradio.Textbox(
                                    label="Input Folder",
                                    placeholder="Enter the full path to the folder containing your images (e.g., C:\\Users\\...\\Pictures)",
                                    info="The folder with the source images to process."
                                )
                                self.ui.bulk.output_folder = gradio.Textbox(
                                    label="Output Folder",
                                    placeholder="Enter the full path to where videos will be saved (e.g., C:\\Users\\...\\Videos\\Output)",
                                    info="A new folder will be created if it doesn't exist. Videos will be named after the input images."
                                )
                            self.ui.bulk.process_button = gradio.Button(
                                value="🔥 Start Bulk Process 🔥",
                                variant="primary"
                            )
                        with gradio.Column(variant="panel"):
                            self.ui.bulk.log = gradio.Textbox(
                                label="Process Log",
                                lines=20,
                                interactive=False,
                                autoscroll=True,
                                placeholder="Bulk process status will be shown here..."
                            )
            ...
            BrokenGradio.progress_button(self.ui.estimate, self.simple(self.estimate, outputs=outputs))
            BrokenGradio.progress_button(self.ui.upscale, self.simple(self.upscale, outputs=outputs))
            ...
            # Start bulk process on button click
            BrokenGradio.progress_button(self.ui.bulk.process_button, self.simple(
                self.render_bulk, outputs={self.ui.bulk.log}),
                concurrency_limit=1 # Only one bulk process at a time
            )
